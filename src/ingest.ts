...
import {
  makeLimiter, getRepo, listMergedPRs, listPRCommitShas,
  listRecentCommits, listIssues, getCommitDetail, GHCommit,
} from "./github";
...
export async function ingestRepo(
  repoId: number, db: Database, cfg: IngestConfig
): Promise<void> {
  ...
  const existingPRNumbers = new Set(
    dbAll(db, "SELECT number FROM pull_requests WHERE repo_id = ?", repoId)
  ...
  for (const pr of prs) {
    if (!existingPRNumbers.has(pr.number)) {
      dbRun(db,
        ...
        repoId, pr.number, pr.title, pr.body, pr.state, pr.merged_at
      );
  ...
  const existingCommitShas = new Set(
    dbAll(db, "SELECT sha FROM commits WHERE repo_id = ?", repoId)
      .map(r => r["sha"] as string)
  );

  const prRows = dbAll(db, "SELECT id, number FROM pull_requests WHERE repo_id = ?", repoId);
  const prIdByNumber = new Map(prRows.map(r => [r["number"] as number, r["id"] as number]));

  const prCommitTasks = prs.map(pr => prLimiter(async () => {
    ...
    await Promise.all(newShas.map(sha => commitLimiter(async () => {
      try {
        ...
        dbRun(db,
          ...
          repoId, sha, detail.commit.author.name, detail.commit.author.date, detail.commit.message, prDbId
        );
        ...
        if (commitRow) {
          ...
          for (const f of (detail.files ?? [])) {
            dbRun(db,
              ...
              commitDbId, f.filename, f.status
            );
          }
        }
      } catch (e) {
  ...
  const latestRow = dbGet(db, "SELECT MAX(date) as d FROM commits WHERE repo_id = ?", repoId);
  const latestDate = latestRow?.["d"] as string | null;

  const recentCommits = await listRecentCommits(
    owner, name, githubToken, cfg.maxUnlinkedCommits, latestDate ?? undefined
  );
  ...
  await Promise.all(unlinkedShas.map(sha => commitLimiter(async () => {
    try {
      const detail = await getCommitDetail(owner, name, sha, githubToken);
      dbRun(db,
        "INSERT OR IGNORE INTO commits (repo_id, sha, author, date, message, pr_id) VALUES (?, ?, ?, ?, ?, ?)",
        repoId, sha, detail.commit.author.name, detail.commit.author.date, detail.commit.message, null
      );
      existingCommitShas.add(sha);
  ...
  // ── 4. Fetch issues ───────────────────────────────────────────────────────
  const issues = await listIssues(owner, name, githubToken);
  const existingIssueNumbers = new Set(
    dbAll(db, "SELECT number FROM issues WHERE repo_id = ?", repoId)
      .map(r => r["number"] as number)
  );
  for (const issue of issues) {
    if (!existingIssueNumbers.has(issue.number)) {
      dbRun(db,
        ...
        repoId, issue.number, issue.title, issue.body, issue.state
      );
    }
  }
  persist();
...
export async function initAndIngestRepo(
  db: Database, cfg: IngestConfig
): Promise<{ repoId: number; alreadyReady: boolean }> {
  const repoData = await getRepo(cfg.owner, cfg.name, cfg.githubToken);

  const existing = dbGet(db, "SELECT id, status FROM repos WHERE full_name = ?", repoData.full_name);
  if (existing) {
  ...
  const repoId = dbRunGetId(db,
    "INSERT INTO repos (owner, name, full_name, default_branch, status) VALUES (?, ?, ?, ?, 'indexing')",
    cfg.owner, cfg.name, repoData.full_name, repoData.default_branch
  );
  persist();
  return { repoId, alreadyReady: false };
}
